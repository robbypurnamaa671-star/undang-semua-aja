...
export default function PublicInvitation() {
  ...
  const royalVariant: "javanese" | "sundanese" | "minangkabau" | "bugis" = isRoyalBugis
    ? "bugis"
    : isRoyalMinangkabau
    ? "minangkabau"
    : isRoyalSundanese
...
Merupakan kehormatan bagi kami apabila Bapak/Ibu/Saudara/i berkenan hadir. Terima kasih 🙏`;
  ...
  const handleWhatsAppContact = () => {
    if (!invitation?.whatsappNumber) return;
    const number = invitation.whatsappNumber.replace(/\D/g, '');
    ...
    >
      ...
      >
        ...
        <div className="fixed bottom-6 right-6 z-40 flex flex-col gap-3">
          ...
          {/* WhatsApp Share (only if no contact number set) */}
          {!invitation.whatsappNumber && (
            <button
